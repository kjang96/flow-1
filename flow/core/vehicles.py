...
class Vehicles:
    ...
    def add(self,
        ...
        for i in range(num_vehicles):
            ...
            self.__vehicles[v_id]["acc_controller"] = \
                acceleration_controller[0](v_id, **acceleration_controller[1])

            # specify the lane-changing controller class
            self.__vehicles[v_id]["lane_changer"] = \
                lane_change_controller[0](v_id, **lane_change_controller[1])
            ...
            self.__vehicles[v_id]["speed_mode"] = \
                self.get_acc_controller(v_id).speed_mode
            self.__vehicles[v_id]["lane_change_mode"] = \
                self.get_lane_changing_controller(v_id).lane_change_mode
        ...
        # collect data for vehicle initialization upon departure
        type_params = {}
        type_params.update(acceleration_controller[1].get(
            "sumo_cf_params", SumoCarFollowingParams()).controller_params)
        type_params.update(lane_change_controller[1].get(
            "sumo_lc_params", SumoLaneChangeParams()).controller_params)
    ...
    def _add_departed(self, veh_id, veh_type, env):
        ...
        speed_mode = self.get_acc_controller(veh_id).speed_mode
        ...
        lc_mode = self.get_lane_changing_controller(veh_id).lane_change_mode
